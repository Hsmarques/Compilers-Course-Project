...
%token LBRAC OUTPUT PARAMSTR PROGRAM RBRAC REPEAT SEMIC THEN UNTIL VAL VAR WHILE 
%token WRITELN LT GT EQ NEQ LEQ GEQ ADD SUB MUL REALDIV PLUS MINUS CALL
%token <s> INTLIT OR AND MOD DIV
%token <s> REALLIT
%token <s> ID
%token <s> STRING
%token <s> RESERVED
...
SimpleExpr: Term {$$=$1;}
		...
		|	SimpleExpr OR Term {$$=insert_double_child(Or,$2,$1,$3,@2.first_line,@2.first_column);}
...
Term:	Factor {$$=$1;}
	...
	|	Term DIV Term {$$=insert_double_child(Div,$2,$1,$3,@2.first_line,@2.first_column);}
	|	Term MOD Term {$$=insert_double_child(Mod,$2,$1,$3,@2.first_line,@2.first_column);}
	|	Term AND Term {$$=insert_double_child(And,$2,$1,$3,@2.first_line,@2.first_column);}
	;


Factor:	NOT	Factor {$$=insert_single_child(Not,$1,$2,@1.first_line,@1.first_column);}
	|	LBRAC Expr RBRAC {$$=$2;}
	...
	|	ID ParamList {$$=insert_double_child(Call,"",insert_char(Id,$1,@1.first_line,@1.first_column),$2,@2.first_line,@2.first_column);}
	;
...
int main(int argc, char** argv){
	...
	if(yyparse()==0){
		...
		global_declaration(env,my_program);
		main_declaration(my_program,env);
	}
	return 0;
}
