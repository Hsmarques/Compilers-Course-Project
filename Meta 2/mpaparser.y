...
%type <node> FormalParamListPrime FormalParams FuncBlock StatPart CompStat StatList StatListPrime 
...
FuncPart:	FuncPart FuncDeclaration SEMIC {$$=insert_double_child(FuncPart,$1,$2);}
		|	{ $$ = NULL;}
		;
...
StatList:	Stat StatListPrime {$$=insert_list($1,$2);}
		;

StatListPrime:	StatListPrime SEMIC Stat {$$=insert_list($1,$3);}
			|	{ $$ = NULL;}
			;
...
WritePList:	LBRAC WritePListPrime WritePListDPrime RBRAC {$$=insert_list($2,$3);}
		;
...
WritePListDPrime:	WritePListDPrime COMMA WritePListPrime {$$=insert_list($1,$3);}
				|	{ $$ = NULL;}
				;
